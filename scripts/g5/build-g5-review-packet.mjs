...
const expected=[...Array.from({length:5},(_,i)=>`C${String(i+6).padStart(4,'0')}`),...Array.from({length:42},(_,i)=>`C${String(i+16).padStart(4,'0')}`)];
if(JSON.stringify(review.map(x=>x.card_id))!==JSON.stringify(expected)) throw new Error('G5 review card IDs must be C0006-C0010 plus C0016-C0057');
...
const blocker={schema_version:'1.0.0',goal:'G5',status:'BLOCKED_ON_HUMAN_EDITORIAL_RELEASE',generated_at_date:DATE,baseline_approved_cards:approved.map(x=>x.card_id),planned_retirements:['C0004','C0005'],pending_card_ids:review.map(x=>x.card_id),human_review_packet:'docs/ops/review/G5_HUMAN_EDITORIAL_REVIEW_PACKET_2026-09-17.json',machine_gate_status:'PASS',human_release_status:'PENDING_47_OF_47',g5_complete:false,prospective_final_active_count:50,external_publish_authorized:false,repository_push_authorized:false,unblock_condition:'Explicit human release decision for C0006-C0010 and C0016-C0057, then versioned retirement of C0004/C0005; C0011-C0015 remain secondary research only; follow with 50-active-card full regression and clean-checkout reproduction.'};
write('docs/ops/review/G5_HUMAN_EDITORIAL_REVIEW_PACKET_2026-09-17.json',packet);write('docs/ops/blockers/G5_HUMAN_EDITORIAL_RELEASE_REQUIRED_2026-09-17.json',blocker);
