...
if(review.length!==45) throw new Error(`G5 review packet requires exactly 45 pending review cards, got ${review.length}`);
const expected=Array.from({length:45},(_,i)=>`C${String(i+6).padStart(4,'0')}`);
if(JSON.stringify(review.map(x=>x.card_id))!==JSON.stringify(expected)) throw new Error('G5 review card IDs must be exactly C0006-C0050');
...
const packet={schema_version:'1.0.0',goal:'G5',packet_type:'FORTY_FIVE_CARD_HUMAN_EDITORIAL_RELEASE_REVIEW',generated_at_date:DATE,approved_baseline:{card_ids:approved.map(x=>x.card_id),human_release_status:'APPROVED',re_review_required:false},pending_card_ids:review.map(x=>x.card_id),diversity_matrix:matrix,cards,release_boundary:{current_public_cards:approved.map(x=>x.card_id),pending_cards_must_not_be_public:review.map(x=>x.card_id),external_publish_authorized:false,repository_push_authorized:false,g5_complete:false},approval_instruction:'Human review must explicitly approve, request changes, or HOLD the 45 pending cards. Machine PASS is not Human Editorial Release.'};
const blocker={schema_version:'1.0.0',goal:'G5',status:'BLOCKED_ON_HUMAN_EDITORIAL_RELEASE',generated_at_date:DATE,baseline_approved_cards:approved.map(x=>x.card_id),pending_card_ids:review.map(x=>x.card_id),human_review_packet:'docs/ops/review/G5_HUMAN_EDITORIAL_REVIEW_PACKET_2026-09-16.json',machine_gate_status:'PASS',human_release_status:'PENDING_45_OF_45',g5_complete:false,external_publish_authorized:false,repository_push_authorized:false,unblock_condition:'Explicit human release decision for C0006-C0050 followed by post-promotion full regression and clean-checkout reproduction.'};
