...
const evidencePath = path.join(ROOT, 'content/korean-expression/evidence/level3-followup-source-v3.json');

const map = JSON.parse(fs.readFileSync(mapPath, 'utf8'));
const registry = JSON.parse(fs.readFileSync(registryPath, 'utf8'));
const evidence = JSON.parse(fs.readFileSync(evidencePath, 'utf8'));

if (evidence.snapshot_id !== 'KOREAN_LEVEL3_FOLLOWUP_EVIDENCE_V3_2026-09-23') throw new Error('follow-up v3 evidence snapshot mismatch');
if (evidence.entries.length !== 2) throw new Error('follow-up v3 evidence must contain exactly two admitted rows');
...
for (const row of evidence.entries) {
  ...
  term.note = '2026-09-23 국립국어원 한국어기초사전 배포 데이터의 정확한 표제어 ID와 뜻풀이를 후속 확인한 섬세 표현.';
