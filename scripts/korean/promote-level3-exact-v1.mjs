...
const evidencePath = path.join(ROOT, 'content/korean-expression/evidence/level3-exact-source-v1.json');

const map = JSON.parse(fs.readFileSync(mapPath, 'utf8'));
const registry = JSON.parse(fs.readFileSync(registryPath, 'utf8'));
const evidence = JSON.parse(fs.readFileSync(evidencePath, 'utf8'));

if (evidence.snapshot_id !== 'KOREAN_LEVEL3_EXACT_EVIDENCE_V1_2026-09-22') throw new Error('Level 3 evidence snapshot mismatch');
if (evidence.entries.length !== 13 || evidence.unresolved_terms.length !== 22) throw new Error('Level 3 evidence admission/HOLD counts drifted');
...
for (const row of evidence.entries) {
  ...
  if (term.level !== 3) throw new Error(`${row.term}: expected Level 3`);
  ...
  term.note = '2026-09-22 국립국어원 한국어기초사전의 정확한 표제어·뜻풀이를 확인한 섬세 표현.';
...
console.log(JSON.stringify({
  status: 'PASS',
  promoted_level3_terms: evidence.entries.length,
  unresolved_level3_terms: evidence.unresolved_terms.length,
