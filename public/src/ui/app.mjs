...
function renderCard(catalog, card, manifest) {
  ...
  const releaseBanner = qs('#release-banner');
  if (releaseBanner) releaseBanner.hidden = manifest.release_mode !== 'REVIEW_ONLY';
  ...
  if (audioAsset) {
    ...
    label.textContent = `${audioAsset.attribution || '발음 음원'} · `;
    attribution.append(label);
    if (audioAsset.source_url) {
      attribution.append(safeExternalLink(audioAsset.source_url, '원본 음원'));
      attribution.append(document.createTextNode(' · '));
    }
    const licenseUrl = /CC BY-SA 4\.0/i.test(audioAsset.license_or_rights_basis || '')
      ? 'https://creativecommons.org/licenses/by-sa/4.0/'
      : null;
    if (licenseUrl) attribution.append(safeExternalLink(licenseUrl, 'CC BY-SA 4.0 라이선스'));
    else attribution.append(document.createTextNode(audioAsset.license_or_rights_basis || '권리 정보 확인 필요'));
  } else {
    audio.hidden = true;
